...
export class MetadataValuesComponent implements OnChanges {
  ...
  hasBrowseDefinition(): boolean {
    return hasValue(this.appConfig.vocabularies);
  }
  ...
  getBrowseRouteSegment(): string {
    const vocab = this.appConfig.vocabularies ?? [];
    const subjectVocab = vocab.find(v => v.filter === 'subject');

    if (subjectVocab.enabled) {
      return subjectVocab.vocabulary;
    }
